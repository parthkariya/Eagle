...
const TicketBookingDetails = () => {
  ...
  const { GetSavedItinerary, itinerary_data, getitinerary_data } =
    useFlightContext();

  console.log("getitinerary_data", getitinerary_data);
  ...
  const GetItinerary = async () => {
    ...
    await GetSavedItinerary(formdata);
  };
  ...
  const handlePhoneChange = (value) => {
    setFormData({ ...formData, cellCountryCode: value });
  };
                ...
                <div className="col-12 col-lg-5 order-1 order-lg-2">
                  ...
                  <div className="row mt-4">
                    ...
                    <div className="border">
                      ...
                      {item?.paxFareBreakUp
                        ?.sort((a, b) => a.paxType - b.paxType) // ensures Adult → Child → Infant
                        .map((pax, index) => {
                          let paxLabel = "";
                          ...
                          return (
                            <div className="row mt-3 w-100" key={index}>
                              <div className="col-6 text-start text-dark">
                                {paxLabel} ({pax?.paxCount || 1})
                              </div>
                              <div className="col-6 text-end text-dark">
                                <div>
                                  Base Fare: <span>&#8377;</span> {pax.baseFare}
                                </div>
                                <div>
                                  Tax: <span>&#8377;</span> {pax.tax}
                                </div>
                                <div className="fw-bold">
                                  Total: <span>&#8377;</span> {totalFare}
                                </div>
                              </div>
                            </div>
                          );
                      ...
                      <div className="row my-3 w-100 ">
                        <div className="col-6 text-start text-dark">
                          Total Taxes (Adult, Child, Infant)
                        </div>
                        <div className="col-6 text-end text-dark">
                          <span>&#8377;</span> {totalTaxes.toFixed(2)}
                        </div>
                      </div>
